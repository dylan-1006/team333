...
function HandGestureRecognition() {
  ...
  const detectGesture = (landmarks) => {
    ...
    const isFingerStraight = (tip, dip, pip, mcp, tolerance = 0.2) => {
      const totalLength = distance(mcp, pip) + distance(pip, dip) + distance(dip, tip);
    ...
    const indexStraight = isFingerStraight(indexTip, indexDIP, indexPIP, indexMCP);
    const middleStraight = isFingerStraight(middleTip, middleDIP, middlePIP, middleMCP);
    const ringStraight = isFingerStraight(ringTip, ringDIP, ringPIP, ringMCP);
    const pinkyStraight = isFingerStraight(pinkyTip, pinkyDIP, pinkyPIP, pinkyMCP);

    // Thumb specific checks
    const thumbRaised = thumbTip.y < wrist.y - 0.1;
    const thumbOut = thumbTip.x > indexMCP.x;
    const thumbStraight = isFingerStraight(thumbTip, thumbIP, thumbMCP, thumbCMC, 0.3);
    ...
    console.log('Hand state:', {
      raised: { index: indexRaised, middle: middleRaised, ring: ringRaised, pinky: pinkyRaised },
      straight: { index: indexStraight, middle: middleStraight, ring: ringStraight, pinky: pinkyStraight },
    ...
    if (indexRaised && middleRaised && !ringRaised && !pinkyRaised && 
        thumbOut && indexStraight && middleStraight) {
      return "No (Three fingers)";
    }

    // THANKS - Four fingers together
    if (indexRaised && middleRaised && ringRaised && pinkyRaised && 
        fingerSpread < 0.15) {
      return "Thanks (Four fingers)";
    }

    // GOOD AFTERNOON - Thumbs up
    if (thumbRaised && thumbStraight && 
        !indexRaised && !middleRaised && !ringRaised && !pinkyRaised) {
      return "Good Afternoon (Thumbs up)";
    }
  ...
  return (
    <div className="gesture-container">
      ...
      <div className="instructions">
        <h3>Show these gestures clearly:</h3>
        <ul>
          <li>👋 <strong>Hello</strong> - Show all five fingers spread wide apart</li>
          <li>✊ <strong>Yes</strong> - Tight fist with all fingers closed</li>
          <li>☝️ <strong>No</strong> - Three fingers: thumb, index, and middle finger</li>
          <li>🤚 <strong>Thanks</strong> - Four fingers close together</li>
          <li>👍 <strong>Good Afternoon</strong> - Clear thumbs up, other fingers closed</li>
        </ul>
        <div className="tips">
